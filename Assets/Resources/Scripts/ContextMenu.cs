...
#pragma warning disable IDE0060 // Удалите неиспользуемый параметр
	...
	public void UseItem()
	{
		...
		{
			DebugLogging.NullParameter<ContainerSlot>(gameObject:gameObject);
			return;
		}
	...
	public void DropItem()
	{
		...
		DragAndDrop.DropItemToWorld(onClickedContainerSlot, onClickedContainerSlot.ContainedItem.Count);
	...
	{
		ReadOnlyCollection<RaycastResult> underPointerGameObejcts = Utils.GetUnderPointerRaycastsResults();
		...
		for (int i = 0; i < underPointerGameObejcts.Count; i++)
		{
			...
			{
				if (onClickedContainerSlot == this.onClickedContainerSlot || onClickedContainerSlot.ConditionProperty == ContainerSlot.Condition.Empty)
				{
					Hide();
					return;
				}
				...
				if (!useButton.interactable && onClickedContainerSlot.ContainedItem.Item is ItemUsable)
				{ useButton.interactable = true; }
				else if (useButton.interactable && onClickedContainerSlot.ContainedItem.Item is not ItemUsable)
